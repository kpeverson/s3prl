...
import s3prl
from s3prl.upstream.prosodyvec.prosodyvec_model import (
    ProsodyvecConfig,
    ProsodyvecModel,
...
def fix_model_cfg(model_cfg, ckpt_wts, num_layers):
    """fix model_cfg to be compatible with ProsodyvecModel"""
...
def load_converted_model(ckpt: str):
    ...
    task_cfg = merge_with_parent(ProsodyvecPretrainingConfig, ckpt_state["cfg"]["task"]) # ckpt_state["task_cfg"])
    model_cfg = merge_with_parent(ProsodyvecConfig, ckpt_state["cfg"]["model"]) # ckpt_state["model_cfg"])
    
    model = ProsodyvecModel(model_cfg, task_cfg, [None]) # ckpt_state["dictionaries_symbols"])
