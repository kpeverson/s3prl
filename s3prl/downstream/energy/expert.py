...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, upstream_rate, downstream_expert, expdir, **kwargs):
        ...
        self.log_energy = self.datarc.get('log_energy', True)
        self.energy_normalization = self.datarc.get('energy_normalization', None)

        self.voiced_frames_only = self.modelrc.get('voiced_frames_only', False)

        self.train_dataset = EnergyDataset('train', h5_dir, self.datarc['meta_data'], glottal_kwargs, upstream_rate=upstream_rate, log_energy=self.log_energy, return_voice_mask=self.voiced_frames_only)
        self.dev_dataset = EnergyDataset('dev', h5_dir, self.datarc['meta_data'], glottal_kwargs, upstream_rate=upstream_rate, log_energy=self.log_energy, return_voice_mask=self.voiced_frames_only)
        self.test_dataset = EnergyDataset('test', h5_dir, self.datarc['meta_data'], glottal_kwargs, upstream_rate=upstream_rate, log_energy=self.log_energy, return_voice_mask=self.voiced_frames_only)
        ...
        if USEBIN:
            ...
            if self.energy_normalization is None:
                self.loss_func = SimpleMSELoss(log_scale=self.log_energy)
            elif self.energy_normalization == 'corpus':
                mean, std = self.train_dataset.norm_stat
                self.loss_func = CorpusNormalizedMSELoss(mean, std, log_scale=self.log_energy)
            elif self.energy_normalization == 'utterance':
                self.loss_func = UtteranceNormalizedMSELoss(log_scale=self.log_energy)
        print(f"[Downstream Expert] Energy normalization: {self.energy_normalization}, Log energy: {self.log_energy}, loss function: {self.loss_func.__class__.__name__}")
    ...
    def log_records(self, mode, records, logger, global_step, **kwargs):
        ...
        import numpy as np
        vis_dir = os.path.join(self.expdir, "vis-energy-hubert-logmse")
...
class SimpleMSELoss(nn.Module):
    ...
    def forward(self, energy_predictions, energy_targets, mask):
        energy_predictions = energy_predictions.masked_select(mask)
        energy_targets = energy_targets.masked_select(mask)
        if self.log_scale:
            energy_targets = torch.log(energy_targets)
        energy_loss = self.loss(energy_predictions, energy_targets)
        return energy_loss
...
class CorpusNormalizedMSELoss(nn.Module):
    ...
    def forward(self, energy_predictions, energy_targets, mask):
        ...
        if self.log_scale:
            energy_targets = torch.log(energy_targets)
        energy_loss = self.loss(energy_predictions, (energy_targets - self.mean) / self.std + self.eps)
        return energy_loss
...
class UtteranceNormalizedMSELoss(nn.Module):
    ...
    def forward(self, energy_predictions, energy_targets, mask):
        ...
        if self.log_scale:
            energy_targets = torch.log(energy_targets)
        mean = torch.mean(energy_targets)
        std = torch.std(energy_targets)
        energy_loss = self.loss(energy_predictions, (energy_targets - mean) / (std + 1e-6))
        return energy_loss
...
class LogMSELoss(nn.Module):
    ...
    def forward(self, energy_predictions, energy_targets, mask):
        energy_predictions = energy_predictions.masked_select(mask)
        energy_targets = energy_targets.masked_select(mask)
        energy_loss = self.loss(energy_predictions, torch.log(energy_targets))
        return energy_loss